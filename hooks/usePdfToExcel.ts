...
import { PdfFileInfo, PdfToExcelResult } from "@/lib/pdf/types";
...
export function usePdfToExcel() {
  ...
  const processConversion = useCallback(async () => {
    ...
    try {
      ...
      setTimeout(() => setStatusMessage("Analyzing tables..."), 800);
      setTimeout(() => setStatusMessage("Extracting spreadsheet data..."), 2000);
      setTimeout(() => setStatusMessage("Generating Excel..."), 3500);
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || "Failed to extract tables from PDF");
      }
      ...
      const tablesDetected = parseInt(response.headers.get("X-Tables-Detected") || "0", 10);

      const originalName = fileInfo.file.name.replace(/\.[^/.]+$/, "");
      const finalFilename = `${originalName}.xlsx`;
